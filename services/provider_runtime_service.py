...
class ProviderRuntimeService:
    ...
    def refresh_if_needed(self) -> None:
        ...
        previous_cfg = getattr(self.manager, "_active_cfg", None)
        previous_error = self.manager._config_error_message

        if previous_cfg is cfg and previous_error is None:
            return
        ...
        session_revocation_service = getattr(
            self.manager, "_session_revocation_service", None
        )
        previous_fingerprint = getattr(
            self.manager, "_sensitive_reload_fingerprint", None
        )
        new_fingerprint = None
        sensitive_policy_changed = False
        if session_revocation_service is not None:
            new_fingerprint = session_revocation_service.sensitive_reload_fingerprint(
                cfg
            )
            sensitive_policy_changed = bool(
        ...
        self.manager._active_cfg = cfg
        self.manager._sensitive_reload_fingerprint = new_fingerprint
        ...
        if (
            ...
            and session_revocation_service is not None
            and cfg.security.enable_session_revocation_on_sensitive_reload
        ):
            changed_epoch = session_revocation_service.bump_epoch(
                reason="sensitive_config_reload",
                details={
                    "config_reload_seconds": cfg.general.config_reload_seconds,
                    "session_revocation_backend": cfg.security.session_revocation_backend,
                },
            )
            log.warning(
                "Sensitive auth configuration changed on reload; stale sessions now require re-authentication (epoch=%s)",
                changed_epoch,
            )
        ...
        if new_config_error:
            ...
            log.info(
                "Auth manager recovered from degraded mode; methods=%s",
                enabled_methods,
            )
