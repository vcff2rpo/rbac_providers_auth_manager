...
class UserSessionService:
    ...
    def serialize_user(self, user: Any) -> dict[str, Any]:
        ...
        claims.update(
            self.manager._session_revocation_service.token_claims_for_new_session()
        )
        return claims
    ...
    def deserialize_user(self, token: dict[str, Any]) -> Any:
        ...
        token_is_current, token_epoch, current_epoch = (
            self.manager._session_revocation_service.validate_token_claims(token)
        )
        if not token_is_current:
            if getattr(self.manager, "_audit_service", None) is not None:
                self.manager._audit_service.log_flow_event(
                    event="auth.session_revocation.rejected",
                    level="warning",
                    principal=subject,
