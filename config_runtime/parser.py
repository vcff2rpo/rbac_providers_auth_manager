...
def load_config(ini_path: Path) -> AuthConfig:
    ...
    if security.session_revocation_backend not in {
        "memory",
        "in_memory",
        "local",
        "redis",
        ...
        raise ValueError(
            "Unsupported security.session_revocation_backend: "
            f"{security.session_revocation_backend}"
        )
    if (
        security.session_revocation_backend == "redis"
        and not (security.session_revocation_redis_url or "").strip()
    ):
        raise ValueError(
            "security.session_revocation_backend=redis requires "
            "security.session_revocation_redis_url to be set"
        )
