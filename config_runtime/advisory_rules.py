...
def effective_session_revocation_backend(value: str) -> str:
    """Return the canonical session-revocation backend name."""
    normalized = (value or "memory").strip().lower()
    if normalized in {"memory", "in_memory", "local"}:
        return "memory"
    return normalized
...
def _rule_non_shared_session_revocation_backend_multi_worker(
    cfg: AuthConfig, *, defined_roles: set[str], airflow_worker_count: int
) -> tuple[ConfigAdvisory, ...]:
    backend = effective_session_revocation_backend(
        cfg.security.session_revocation_backend
    )
    if not (
        cfg.security.enable_session_revocation_on_sensitive_reload
        and airflow_worker_count > 1
        and backend == "memory"
    ):
        return ()
    return (
        ConfigAdvisory(
            severity="warning",
            code="non_shared_session_revocation_backend_multi_worker",
            message=(
                "security.enable_session_revocation_on_sensitive_reload=true is configured while "
                "security.session_revocation_backend=memory and Airflow API workers="
                f"{airflow_worker_count}. Forced logout state will not be shared across workers. "
                "Prefer the Redis session revocation backend for enterprise deployments."
            ),
        ),
    )
...
ADVISORY_RULES: tuple[ConfigAdvisoryRule, ...] = (
    ...
    _rule_non_shared_session_revocation_backend_multi_worker,
