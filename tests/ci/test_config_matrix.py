...
def test_load_config_rejects_invalid_rate_limit_backend(tmp_path: Path) -> None:
    config_path = _write_config(
        tmp_path,
        BASE_CONFIG.replace(
            "rate_limit_backend = memory", "rate_limit_backend = broken"
        ),
    )
...
def test_load_config_requires_redis_url_when_redis_backend_is_selected(
    tmp_path: Path,
) -> None:
    config_path = _write_config(
        tmp_path,
        BASE_CONFIG.replace(
            "rate_limit_backend = memory", "rate_limit_backend = redis"
        ),
    )
...
def test_load_config_rejects_allow_self_signed_without_security_override(
    tmp_path: Path,
) -> None:
    config_path = _write_config(
        tmp_path,
...
def test_load_config_rejects_entra_graph_fallback_without_security_override(
    tmp_path: Path,
) -> None:
    config_path = _write_config(
        tmp_path,
...
def test_role_mapping_parser_supports_legacy_and_recommended_orientation(
    tmp_path: Path,
) -> None:
    config_path = _write_config(
        tmp_path,
...
""".strip(),
    ...
    assert mapping.dn_to_roles["cn=legacy_group,ou=groups,dc=example,dc=com"] == {
        "Viewer"
    }
